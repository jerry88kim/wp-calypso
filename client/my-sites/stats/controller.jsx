...
module.exports = {
	...
	site: function( context, next ) {
		...
		if ( ! activeFilter ) {
			...
			const siteOffset = getSiteOption( context.store.getState(), siteId, 'gmt_offset' );
			const momentSiteZone = i18n.moment().utcOffset( siteOffset ? siteOffset : 0 );
	...
	summary: function( context, next ) {
		...
		if ( siteFragment && 0 === siteId ) {
			...
			const gmt_offset = getSiteOption( context.store.getState(), siteId, 'gmt_offset' );
			if ( gmt_offset || gmt_offset === 0 ) {
				momentSiteZone = i18n.moment().utcOffset( gmt_offset );
			}
			if ( queryOptions.startDate && i18n.moment( queryOptions.startDate ).isValid ) {
